...
Die Eingabedaten für das Clustering-Verfahren bestehen für jede Ressource aus der Menge der für sie verfügbaren Attribute. Es gibt insgesamt ca. 44000 mögliche Attribute in der DBPedia. Da alle Ressourcen aus der Ontologie stammen, kann man davon ausgehen, dass gleiche Attribute die gleiche Semantik haben. Dadurch lassen sich aus dem Ergebnis des Clustering-Verfahrens nach diesen Attributen möglicherweise semantische Rückschlüsse ziehen.
...
Da die Clusterzentren gemittelte Werte enthalte, können sie nicht wie die Subjekte als einfache Mengen dargestellt werden. Stattdessen werden Fuzzy-Mengen benutzt. Bei Fuzzy-Mengen kann ein Element zu einem gewissen Grad zwischen 0 und 1 enthalten sein. Dies wird durch die Funktion $\mu : M \times E \rightarrow [0,1] $ beschrieben. Dabei bedeutet $m(A,e) = 0$ soviel wie $ e \not\in A $ und $m(A,e) = 1$ entspricht $ e \in A $ aus der normalen Mengenlehre.
...
Damit lässt sich der Jaccard-Index

$$J_{\delta}(A,B) = \sum_{p \in P}\frac{max\bigl(\mu(p, A)\mu(p, B)\bigr) - min\bigl(\mu(p, A),\mu(p, B))\bigr)}{max\bigl(\mu(p, A),\mu(p, B))\bigr)}$$
...
Die Knoten können jederzeit auf die lokalen Dateien lesend zugreifen. In unserer Implementierung werden die Clusterzentren vor der Erzeugung der Mapper auf den Knoten ausgelesen und anschließend im Hauptspeicher gehalten.
% "mit der Erzeugung" statt "vor der Erzeugung" ?
...
muss der in \ref{sec:BerechnungDerCluster} beschriebene Prozess mehrmals ausgeführt werden. Allerdings genügt es nicht, 
nur den Job, der die beschriebenen Map- und Reduce-Funktionen enthält, erneut anzustoßen. Zuvor muss die Ausgabe der Reduce-Funktion, 
